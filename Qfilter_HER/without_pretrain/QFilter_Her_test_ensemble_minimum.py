...
import random
import pickle
import TD3_HER_Qfilter_ensemble as TD3
...
while steps < max_steps + 1:
    ...
    if len(replay_buffer) > learning_starts:
        ...
        goal_stats = finalize(goals_agg)
        num_accept, num_total = agent.train(replay_buffer, demos, normalizers=(state_stats[0], np.sqrt(state_stats[1]), goal_stats[0],
    ...
    if episodes % eps_eval == 0:
        ...
        print("Acceptance Rate of Demos = %.2f " % (percent_accept_demos[-1]))
        num_accept_demos = 0
        num_tot_demos = 0

    episodes += 1
np.save(f"/home/zhu_y@WMGDS.WMG.WARWICK.AC.UK/PycharmProjects/pythonProject/Results/{env_name}/EnsQfilter/Noise{var}/{method}/EnsSize_{ensemble_size}_S{seed}_score", score_history)
np.save(f"/home/zhu_y@WMGDS.WMG.WARWICK.AC.UK/PycharmProjects/pythonProject/Results/{env_name}/EnsQfilter/Noise{var}/{method}/EnsSize_{ensemble_size}_S{seed}_success", success_history)
np.save(f"/home/zhu_y@WMGDS.WMG.WARWICK.AC.UK/PycharmProjects/pythonProject/Results/{env_name}/EnsQfilter/Noise{var}/{method}/EnsSize_{ensemble_size}_S{seed}_demoaccept",
        percent_accept_demos)
