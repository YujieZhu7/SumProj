...
class Agent(object):
    def __init__(self, state_dim, action_dim, max_action, hidden_dim=(256,256),
                 ...
                 policy_noise=0.2, noise_clip=0.5, policy_freq=2, device="mps:0"):
    ...
    def train_demos(self, replay_buffer, demos, iterations=2):
        for it in range(iterations):  # update twice = train twice
            ...
            if self.total_it % self.policy_freq == 0:
                ...
                # Q2 = self.critic.Q2(state, policy_actions)
                demos_Q_dem = self.critic.Q1(demos_state, demos_action)
                # Behavior cloning loss is computed only on the demonstrations
                # torch.ge(input, other) = True if input > other
                BC_loss = ((torch.ge(demos_Q_dem, demos_Q)*torch.mean((demos_policy_actions - demos_action)**2,1)).sum()/
                           (torch.ge(demos_Q_dem, demos_Q).sum()+1e-6))
                # (policy_actions - action)**2 computes the element-wise squared difference between the policy actions and the demonstration actions.
                # why get the mean for each row instead of sum? same idea as MSE, normalise across dimensions.
                # the denominator normalises the summed loss by the number of valid comparisons
                # 1e-6 is added to avoid division by zero, ensuring numerical stability.
                actor_loss = -Q.mean() + BC_loss
                # didn't use lamda1 and lamda2 to balance two losses.
