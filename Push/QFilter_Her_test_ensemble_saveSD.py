...
import random
import pickle
import TD3_HER_Qfilter_ensemble_saveSD as TD3
...
while steps < max_steps + 1:
    ...
    if len(replay_buffer) > learning_starts:
        ...
        goal_stats = finalize(goals_agg)
        num_accept, num_total, demos_Q_std, Q_dem_std = agent.train(replay_buffer, demos,
        ...
        demos_std.append(demos_Q_std)
        policy_std.append(Q_dem_std)
    ...
    if episodes % eps_eval == 0:
        ...
        if len(percent_accept_demos) == 0:
            ...
            print("Demo SD %.2f" % demos_std[-1], "Policy SD %.2f" % policy_std[-1])
...
np.save(f"/home/zhu_y@WMGDS.WMG.WARWICK.AC.UK/PycharmProjects/pythonProject/Results/{env_name}/EnsQfilter/RandGausNoise/{method}/EnsSize_{ensemble_size}_S{seed}_demoaccept",
        average_accept_demos)
np.save(f"/home/zhu_y@WMGDS.WMG.WARWICK.AC.UK/PycharmProjects/pythonProject/Results/{env_name}/EnsQfilter/RandGausNoise/{method}/EnsSize_{ensemble_size}_S{seed}_demoSD",
        demos_std)
np.save(f"/home/zhu_y@WMGDS.WMG.WARWICK.AC.UK/PycharmProjects/pythonProject/Results/{env_name}/EnsQfilter/RandGausNoise/{method}/EnsSize_{ensemble_size}_S{seed}_policySD",
        policy_std)
